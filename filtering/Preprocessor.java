...
public class Preprocessor {
    public static String generate_data(String key, String value, String status) {
        Timestamp timestamp = new Timestamp(System.currentTimeMillis());
        String type = key.contains("temp") ? "temp" : "humid";
        return "{\"" + "sensor_name\":\"" + key + "\",\"" + type + "\":" + value + ",\"status\":\"" + status + "\",\"timestamp\":\"" + timestamp.toString() + "\"}";
    }
    ...
    public static void main(String[] args) {
        ...
        KStream<String, String> sample_records = valid_records
                // Random sample, 50% dropout
                .filter((k, v) -> r.nextDouble() < 0.5 || !is_sample)
                // Synchronous sample
                .filter((k, v) -> {
                    Timestamp now = new Timestamp(System.currentTimeMillis());
                    // Interval of 3 second with a 0.5 second margin
                    return (now.getTime() - start.getTime()) % 3000 < 500 || !is_sample;
                });
        ...
        Map<String, KStream<String, String>> branch = sample_records.split()
                // Temperature within (10,60) is in a safe temperature range
                .branch((String sensor_name, String value) -> sensor_name.contains("temp") && Double.valueOf(value) > 10.0 && Double.valueOf(value) < 60.0,
                        Branched.withConsumer(ks -> ks.map((key, value) -> KeyValue.pair(key, generate_data(key.toString(), value.toString(), "safe"))).to("preprocessed-temperature")))
                // Humidity within (20,70) is in a safe humidity range
                .branch((String sensor_name, String value) -> sensor_name.contains("humid") && Double.valueOf(value) > 20.0 && Double.valueOf(value) < 70.0,
                        Branched.withConsumer(ks -> ks.map((key, value) -> KeyValue.pair(key, generate_data(key.toString(), value.toString(), "safe"))).to("preprocessed-humidity")))
                // Temperature lower than 10 or higher than 60 is unsafe
                .branch((String sensor_name, String value) -> sensor_name.contains("temp"),
                        Branched.withConsumer(ks -> ks.map((key, value) -> KeyValue.pair(key, generate_data(key.toString(), value.toString(), "unsafe"))).to("preprocessed-temperature")))
                // Humidity lower than 20 or higher than 70 is unsafe
                .branch((String sensor_name, String value) -> sensor_name.contains("humid"),
                        Branched.withConsumer(ks -> ks.map((key, value) -> KeyValue.pair(key, generate_data(key.toString(), value.toString(), "unsafe"))).to("preprocessed-humidity")))
                .noDefaultBranch();
